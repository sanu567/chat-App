     ...
     max-sm:flex-col-reverse rounded-lg">
      <form onSubmit={HandleSubmit} className="flex flex-col gap-5 p-10 flex-1">
        ...
        <label htmlFor="avatar" className="flex items-center gap-3 cursor-pointer">
          ...
          <img src={selectedImg ? URL.createObjectURL(selectedImg): <IoPersonSharp />} alt=""
          className={`w-12 h-12 ${selectedImg && 'rounded-full'}`} />
          Upload profile image
        </label>
        <input onChange={(e)=>{setName(e.target.value)}} value={name}
        type="text" required placeholder="Your name" className="p-2 border border-gray-500
        roundede-md focus:outline-none focus:ring-2 focus:ring-violet-500 bg-transparent " />
      ...
      <img src={authUser.profilePic || logo} alt="" className={`max-w-44 aspect-square rounded-full max-10 max-sm:mt-10 ${selectedImg && 'rounded-full'}`} />
     </div>
